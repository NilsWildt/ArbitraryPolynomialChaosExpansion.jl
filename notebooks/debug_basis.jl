...
create_basis(x, degree,onb; normalize_data=true) = create_basis(x,degree,Val(onb);normalize_data=normalize_data)

# ╔═╡ a5349a70-55d4-4610-8c54-7e29d4315467
function create_vandermonde(x::Vector)
    n = length(x)
    # Create the Vandermonde matrix
    vandermonde_matrix = [x[i]^(j-1) for i in 1:n, j in 1:n]
...
MultivariatePolynomialDegrees = aPCE_MultivariatePolynomialDegrees(InputDimensions, degree, 1.0, 1.0)
	NumberOfTerms = min(size(MultivariatePolynomialDegrees, 1), numberPolynomials(degree, InputDimensions))  
	OrthonormalBasis = create_basis(x, degree,true; normalize_data=true)
	# UnicodePlots.spy(Matrix(sparse(OrthonormalBasis)))
	@info "" mean(OrthonormalBasis) 
...
MultivariatePolynomialDegrees = aPCE_MultivariatePolynomialDegrees(InputDimensions, degree, 1.0, 1.0)
	NumberOfTerms = min(size(MultivariatePolynomialDegrees, 1), numberPolynomials(degree, InputDimensions))  
	OrthonormalBasis = create_basis(x, degree,false; normalize_data=true)
	# UnicodePlots.spy(Matrix(sparse(OrthonormalBasis)))
	@info "" mean(OrthonormalBasis) 
	Ψ_train = aPCE_PsiPolynomialMatrix(x, MultivariatePolynomialDegrees, OrthonormalBasis)'
	return Ψ_train
...
function get_V_vandermonde(x)
return create_vandermonde(x)
end
...
let
		...
		V3 = get_V_vandermonde(TrainingInput) 
	  ...
	  #   	@tensor coeffs[i, k] = Psi_inv[i, j] * TrainingOutput[j, k]
			...
			TensorOperations.@tensor PredictionOutput[k, j] := V[i, k] * coeffs[i, j]
			@info mean((PredictionOutput.-TrainingOutput).^2)
