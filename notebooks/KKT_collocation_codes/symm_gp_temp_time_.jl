...
# ╔═╡ 6829c0fe-d726-4bdb-af6b-9cff909c4188
# function K(t_u,x_u, tp_u, xp_u,θ)
# 	Kl = θ[1]*SqExponentialKernel()
# 	X_u = [t_u x_u]
# 	Y_u = [tp_u xp_u]
# 	# K_points(x_t,x_u,xp_t, xp_u,θ) = θ[1]*exp(-1/2*((x_t-xp_t)^2+(x_u-xp_u)^2))
# 	 # K = θ[1].*exp.(-0.5.*(pwD2(X,Y)))
# 	return kernelmatrix(K,X_u',Y_u')
# end

# ╔═╡ 5f0f9136-c65b-433b-a288-bc6143567910
# ∂K∂x_tu.(t_u)

# ╔═╡ c61c0c5f-03be-43d7-87d7-a74442c2b3f5
# begin
# ∂K∂x_tu(x::Float64) = ForwardDiff.derivative(x -> K(x,x_u, t_u ,x_u,θ), x)
# ∂K∂y(x,y) = ForwardDiff.derivative(x -> K(x,y,θ), y)
# ∂∂K∂²x(x,y) = ForwardDiff.derivative(x -> ∂K∂x(x,y)(x,y), x)
# ∂∂K∂²y(x,y) = ForwardDiff.derivative(x -> ∂K∂y(x,y)(x,y), y)
# ∂∂K∂x∂y(x,y) = ForwardDiff.derivative(x -> ∂K∂x(x,y)(x,y), y)
# ∂∂K∂y∂x(x,y) = ForwardDiff.derivative(x -> ∂K∂y(x,y)(x,y), x)
# end
# ∇K = x -> ForwardDiff.derivative(K, x)
#     ΔK = x -> ForwardDiff.derivative(∇K, x)

# ╔═╡ 7c29773b-21ec-4430-ac52-99c4cc922bd7
# begin
...
# ╔═╡ 9ad2f64c-3546-443e-9ff6-00741a7f3895


# ╔═╡ bd49aedb-e942-46e4-a58e-6fcb82fbbdc5
...
begin # Exp cov definition
	abstract type CovarianceFunction end	
	struct ExponentialCov <: CovarianceFunction end
	
	function evalCOV(expCov::ExponentialCov, h)
	    return @. exp(-abs(h)^2)
	end
	
	_evalCovFunction(expCov::ExponentialCov, xy_dist) = @. exp(-abs(xy_dist))
	
	function evalCovMatrix(expCov::ExponentialCov, x::AbstractArray, y::AbstractArray)
		xy_dist = pwD2(x, y)
		K = _evalCovFunction(expCov, xy_dist)
	end

	function get_σ_matrix(expCov::ExponentialCov, t::AbstractArray,t0::Float64)
		σ = [evalCOV(expCov,ti.-t0) for ti in t]
	end
...
function evalKmatrix(epanechnikov::Epanechnikov, x::AbstractArray, y::AbstractArray)
    xy_dist = pwD2(x, y)
    K = _evalKmatrix(epanechnikov, xy_dist) # Do the inner product and get the polynomial Kernel Matrix
end
...
Base.String(::Polynomial) = "k_polynomial"
	...
	mutable struct Exponential <: Kernel
	    "Scaling"
	end
	...
	_evalKmatrix(exponential::Exponential, xy_dist) = @fastmath @. exp(-xy_dist)
	
	function evalKmatrix(exponential::Exponential, x::AbstractArray, y::AbstractArray)
	    xy_dist = pwD2(x, y)
	    K = _evalKmatrix(exponential, xy_dist) # Do the inner product and get the polynomial Kernel Matrix
	end
	
	function evalKernel(exponential::Exponential, x)
	    return   @. exp(-abs(exponential.h)^2)
	end
	
	Base.String(::Exponential) = "Exponential"
...
git-tree-sha1 = "0fce9a8cacf5077f1911f9b2b1667d263cd92e74"
