...
# ╔═╡ 72d339f4-0219-40f0-b925-ec1011fbccd9
# function f(x)  # Calculated RHS
# 	x1 = x[1]
# 	x2 = x[2]
# 	return @. -pi^2* ( sin(pi*x1)*sin(pi*x2) +64*sin(4*pi*x1)*sin(4*pi*x2) + sin(pi*x1)*sin(pi*x2)+64*sin(4*pi*x1)*sin(4*pi*x2))
# end
...
M, ΔMC, ΔΔMC = k_matern_derivatives(k_matern_quadratic, ε);

# ╔═╡ 9e480b96-26ec-4a9d-92d1-aeeb8e615fe6
...
XΩ, X∂Ω = collocation_points(1000, 600);
...
Λ = collocation_matrix(M, ΔMC, ΔΔMC, XΩ, X∂Ω) + 1.0e-13I;

# ╔═╡ 97f8c31f-fba1-444b-b688-3772a4c09c44
Xnew, _ = collocation_points(N, 64);
...
begin
    ...
    ax = Axis3(
        fig[1, 1], aspect = (1, 1, 1), viewmode = :stretch, perspectiveness = 0.6, elevation = π / 18, azimuth = π / 3,
    ...
    surface!(ax, XΩ[:, 1], XΩ[:, 2], u_true(XΩ[:, 1], XΩ[:, 2]), colormap = (:Blues, 0.2), transparency = false, shading = false)
...
# ╔═╡ 8ff532ef-fcd6-4d38-a26f-7169f2f20939
