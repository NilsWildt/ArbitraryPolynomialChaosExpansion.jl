...
function ex_info(MVPD)
    # Calculate the required metrics
    same_order_count = count_same_order(MVPD)
    cross_order_count = count_cross_order(MVPD)
    double_order_count = count_and_sort_elements_by_value(MVPD)
    mean_order = mean_orders(MVPD)
    mean_of_means = mean(mean_order)
    var_of_means = var(mean_order)
    ...
    @info "Double order:" double_order_count
    @info "Mean order:" mean_order
    @info "Mean mean:" mean_of_means
    @info "Var mean:" var_of_means
...
function compare(f, qnorm1=0.1, dim=3, maxorder=6)
    ...
    try
        display(reduce(hcat, intersect(s1, s2)))
    catch
        @warn "No intersection found or an error occurred during intersection display"
    end
    display("----------Difference-------------------------------")
    try
        display(reduce(hcat, setdiff(s2, s1)))
    catch
        @warn "No difference found or an error occurred during difference display"
    end
    result_df = ex_info(MVPD1)
    result_df.qnorm = fill(qnorm1, nrow(result_df))
    result_df.dim = fill(dim, nrow(result_df))
    result_df.maxorder = fill(maxorder, nrow(result_df))
    return result_df
end
    ...
    for qnorm1 in qnorm_values
        for dim in dims
            for maxorder in maxorders
                result_df = compare(f, qnorm1, dim, maxorder)
                results = vcat(results, result_df)
            end
        end
    end
...
git-tree-sha1 = "b07c6de9de496d398eb1aab835c80a20c440c6ba"
