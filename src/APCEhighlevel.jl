...
@stable function train!(aPCE, TrainingInput, y_rhs; bayesian_inversion=:true, reg_mode=3)
    ...
    if bayesian_inversion
        ...
        for i in axes(y_rhs, 2)
            ...
            aPCE.ExpansionCoefficients[:, i] .= invert(Psi, y_rhs[:, i], Lₖx₀(reg_mode, view(x₀, :, i)); alg=:gcv_svd, method=LBFGS(linesearch=LineSearches.BackTracking()))
        end
    end
...
@stable function create_Polynomial_Degrees(input_dimensions, degree, s_marginals=1.0, s_interactions=1.0)
    MultivariatePolynomialDegrees = aPCE_MultivariatePolynomialDegrees(input_dimensions, degree, s_marginals, s_interactions)
