function [dmk,Mflux,out] = CSARWPTconstdL(qx,qy,qz,grid,X0,readx,Disp,ne_em,nen,mod,test)
%
...
if mod == 3
    C           = zeros(prod(N),1);       % concentration field, plume
    % concentration plume
...
NPT = sum(npe);
compromise = 10;
Xallt       = zeros(10*NPT,Dim);
point       = zeros(10*NPT,1);
recorded    = 0;
Tp      = zeros(NPT,1);
T       = zeros(NPT,1);
...
%     Xallt(count:count+sum(checkt)-1,:)    = Xi_1(checkt,:) .* l./dt(checkt,:) + X(checkt,:) .* (1-l./dt(checkt,:)); %Xallt(look(checkt)+round(T(checkt)/gdt).*size(X,1),:) = Xi_1(checkt,:) .* l./dL + X(checkt,:) .* (dL-l)./dL;
    ...
    if size(Xallt,1) > compromise*NPT
         recorded = recorded + size(Xallt,1);
         disp([ num2str(recorded) ' particles recorded' datestr(now)]) 
        q = 1/NP;%(NPT)^-1;  
...
for lime = 1:size(X0,1)
    ...
    %[kde]       = kdefun(x,x_PDF,1e-3,ones(1,size(x,2)).*q,bwk',1,[0;0;0],[N.*lambda]'); % kdefun calls either figtree or gpuml
    [kde]       = kdefun(x,x_PDF,1e-3,ones(1,size(x,2)).*q,bwk',1,[0;0;0],[N.*lambda]'); % kdefun calls either figtree or gpuml
...
%                 figure(1), hold on, plot3(X(ind,2),X(ind,1),X(ind,3),'.','Color','k'),
...
% Cij     = Cij./P;
Cij      = (Cij+ Mij)./neem .*gdt;%[s/m^3]
epsi     = Cij\cs;
% epsi     = (cs'*(Mij.*neem))/Cij;%Cij\(Mij.*neem)*cs;
dmk      = epsi; % because P == m0 and dmk/m0 == epsi
Mflux    = sum(dmk);
...
if mod == 3;
    cutter =10;
    cutat  = size(Xallt,1)/cutter;
    for cut = 1:cutter
        
       if cut < cutter
            x       = Xallt((cutat*(cut-1)+1):(cutat*cut),:);
            x = x';
            poin    = point((cutat*(cut-1)+1):(cutat*cut));
        else
            x       = Xallt((cutat*(cut-1)+1):end,:);
            x = x';
            poin    = point((cutat*(cut-1)+1):end);
        end 
%     x           = [Xallt(:,1)';Xallt(:,2)';Xallt(:,3)'];
    ...
    [kde]   = kdefun(x,x_pdf',1e-3,q,[bwk],0,[0;0;0],[N.*lambda]'); % kdefun calls either figtree or gpuml
    %                     C       = C + kde.* dt .* size(x,2) .* mdach ./ (ne);
    
    C      = C + kde;
    
%     %easy controlleti----------------------------------------------------------
%     io = sub2ind(N, X0(:,1)./lambda(1)+0.5, X0(:,2)./lambda(2)+0.5, X0(:,3)./lambda(3)+0.5);
%     if  any(round(C(round(io)).*10)<5) %|| any(round(C(io).*10)>5)
%     disp('boundary condition not matched')
%     [controlleti]   = kdefun(x,X0',1e-3,q,bwk,0,[0;0;0],[N.*lambda]'); 
%     controlleti     = controlleti.*gdt./(neem(:,size(X0,1)) .* V);
%     end
    %--------------------------------------------------------------------------
    end
    out = C.*gdt./(ne.*V);
