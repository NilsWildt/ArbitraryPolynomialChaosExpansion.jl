...
%% Visualization: mean value of model output
figure(1)
subplot(1,2,1)
line1=plot(OutputMean); 
set(line1, 'Color', [0 0 0], 'LineWidth', 1, 'MarkerSize', 2);  
h= findobj(gca,'Type','patch');
set(h,'FaceColor',[0.5 0.5 0.5],'EdgeColor',[0. 0. 0.]);
grid on;
box on;
xlabel('Time');
ylabel('Mean value of model output');
subplot(1,2,2)
line2=plot(sqrt(OutputVar));
set(line2, 'Color', [0 0 0], 'LineWidth', 1,  'MarkerSize', 2);  
h= findobj(gca,'Type','patch');
set(h,'FaceColor',[0.5 0.5 0.5],'EdgeColor',[0. 0. 0.]);
grid on;
box on;
xlabel('Time');
ylabel('Standard deviation of model output');
set(gca,'Color',[1 1 1])
set(gcf,'Color',[1 1 1]);
set(gcf,'Position',[100 600 750 300]);
